...
const label = "block text-[10px] text-gray-400 uppercase tracking-[1.5px] mb-1.5 font-semibold";

// ─── Toast notification ────────────────────────────────────────────────────────
function Toast({ message, onDone }) {
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 bg-emerald-700 text-white text-sm font-semibold px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-2"
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        ...
        <div>
          <h2 className="font-bold text-gray-800 text-[15px]" style={{ fontFamily: "'Playfair Display', serif" }}>
            {title}
          </h2>
...
// 1. RESTAURANT INFO
// ═══════════════════════════════════════════════════════════════════════════════
function RestaurantInfo({ onSave }) {
...
function TaxSettings({ onSave }) {
  ...
  const toggle = (id) =>
    setTaxes((p) => p.map((t) => t.id === id ? { ...t, enabled: !t.enabled } : t));
  ...
  const addTax = () => {
    ...
    setTaxes((p) => [...p, { id: Date.now(), name: newTax.name, rate: newTax.rate, enabled: true }]);
    setNewTax({ name: "", rate: "" });
    setShowAdd(false);
  };

  const removeTax = (id) => setTaxes((p) => p.filter((t) => t.id !== id));
        ...
        {taxes.map((t) => (
          <div key={t.id}
            ...
            <button onClick={() => toggle(t.id)}
                    className={`w-10 h-5.5 rounded-full relative transition-colors shrink-0 ${t.enabled ? "bg-emerald-500" : "bg-gray-300"}`}
                    style={{ width: 40, height: 22 }}>
              <span className={`absolute top-0.5 w-4.5 h-4.5 bg-white rounded-full shadow transition-all ${t.enabled ? "left-4.5" : "left-0.5"}`}
        ...
        {showAdd ? (
          <div className="flex items-center gap-2 p-3.5 rounded-xl border border-dashed border-emerald-300 bg-emerald-50">
            ...
            <button onClick={addTax}
                    className="px-3 py-1.5 bg-emerald-700 text-white text-xs font-bold rounded-lg hover:bg-emerald-800 transition-all">Add</button>
...
function OperatingHours({ onSave }) {
  const [hours, setHours] = useState(
    DAYS.map((day, i) => ({
      day,
      open:  "09:00",
      close: "22:00",
      closed: i === 6, // Sunday closed by default
    }))
  );
  ...
  return (
    <Section icon="🕐" title="Operating Hours" subtitle="Set your restaurant's open and close times">
      <div className="flex flex-col gap-2">
        {hours.map((h, idx) => (
          <div key={h.day}
               className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${h.closed ? "border-gray-100 bg-gray-50 opacity-60" : "border-gray-100 bg-white"}`}>
            {/* Day name */}
...
function TableManagement({ onSave }) {
  ...
  const toggleActive = (id) =>
    setTables((p) => p.map((t) => t.id === id ? { ...t, active: !t.active } : t));

  const addTable = () => {
    if (!newTable.number) return;
    setTables((p) => [...p, { id: Date.now(), ...newTable, capacity: parseInt(newTable.capacity), active: true }]);
    setNewTable({ number: "", capacity: "4", type: "Indoor" });
    setShowAdd(false);
  };

  const typeColor = { Indoor:"bg-blue-50 text-blue-600", Outdoor:"bg-green-50 text-green-600",
                      Private:"bg-purple-50 text-purple-600", Bar:"bg-amber-50 text-amber-600",
                      Rooftop:"bg-orange-50 text-orange-600" };

  return (
    <Section icon="🪑" title="Table Management" subtitle="Add, edit, or remove dining tables">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {tables.map((t) => (
          <div key={t.id}
               className={`relative p-4 rounded-xl border transition-all ${t.active ? "border-gray-100 bg-white shadow-sm" : "border-gray-100 bg-gray-50 opacity-60"}`}>
            {/* Remove btn */}
            <button onClick={() => removeTable(t.id)}
                    className="absolute top-3 right-3 text-gray-300 hover:text-red-400 transition-colors text-base leading-none">✕</button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                  <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${typeColor[t.type] || "bg-gray-100 text-gray-500"}`}>
                    {t.type}
                  </span>
            ...
            <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
              <span className="text-xs text-gray-400 font-medium">{t.active ? "Active" : "Inactive"}</span>
              <button onClick={() => toggleActive(t.id)}
                      className={`relative rounded-full transition-colors`}
                      style={{ width: 36, height: 20, background: t.active ? "#059669" : "#d1d5db" }}>
        ...
        {showAdd ? (
          <div className="p-4 rounded-xl border border-dashed border-emerald-300 bg-emerald-50 flex flex-col gap-2.5">
            ...
            <input className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-emerald-400"
                   placeholder="Table number (e.g. T7)" value={newTable.number}
            ...
            <div className="grid grid-cols-2 gap-2">
              <input type="number" min="1" max="20"
                     ...
                     placeholder="Seats" value={newTable.capacity}
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="w-full bg-emerald-700 px-4 sm:px-8 py-4 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-xl sm:text-[26px] font-serif text-white tracking-wide">Settings</h1>
          <p className="text-[10px] sm:text-[11px] text-white/70 tracking-[2px] uppercase mt-0.5 font-semibold">
      ...
      <div className="flex min-h-[calc(100vh-76px)]">
        ...
        {/* ── Sidebar ── */}
        <aside className="w-56 shrink-0 bg-white border-r border-gray-100 shadow-sm hidden sm:flex flex-col py-4 gap-1 px-2 sticky top-0 h-screen">
        ...
        {/* ── Content area ── */}
        <main className="flex-1 px-4 sm:px-8 py-6 pb-24 sm:pb-6 overflow-y-auto"
