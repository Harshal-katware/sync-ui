...
//  *                   "dashboard" → existing dark/blurred look with settings + account dropdown
//  *                   "module"    → solid emerald-700 background, module name shown, no dropdowns
//  *  - moduleName   : string — shown when variant="module"   e.g. "Inventory Control"
//  *  - moduleSubtitle: string — shown below moduleName       e.g. "Restaurant Management System"
//  *
//  * Usage:
//  *   <Navbar variant="dashboard" />                                    // Dashboard page
//  *   <Navbar variant="module" moduleName="Inventory Control" />        // Inventory page
//  *   <Navbar variant="module" moduleName="Billing" />                  // Billing page
//  */
...
//   useEffect(() => {
//     const handler = (e) => {
//       if (accountRef.current  && !accountRef.current.contains(e.target))  setOpenMenu((m) => m === "account"  ? null : m);
//       if (settingsRef.current && !settingsRef.current.contains(e.target)) setOpenMenu((m) => m === "settings" ? null : m);
//     };
...
//       {showLogout   && (
//         <LogoutConfirm
//           onConfirm={() => { setShowLogout(false); /* add logout/navigate logic */ }}
...
type MenuType = "settings" | "account" | null;
...
function ChangePasswordModal({ onClose }: ChangePasswordModalProps): JSX.Element {
  const [form, setForm] = useState<ChangePasswordForm>({
    current: "",
    newPass: "",
    confirm: "",
  });
  const [show, setShow] = useState<ChangePasswordShow>({
    current: false,
    newPass: false,
    confirm: false,
  });
...
export default function Navbar({
  ...
  const adminName: string = "Yuvraj Patil";
  ...
  const toggleMenu = (menu: MenuType): void =>
      ...
      <div className="w-full bg-white/5 backdrop-blur-sm px-4 sm:px-8 py-4 flex items-center justify-between gap-3 text-[#b2d1df]">
        ...
        <div className="flex items-center gap-2 relative">
          ...
          <div className="relative" ref={settingsRef}>
            ...
            {openMenu === "settings" && (
              <div
                className="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-gray-100 z-50 overflow-hidden py-2"
                style={{ animation: "cpSlideDown .2s ease" }}
              >
                <button className="w-full flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors text-left">
                  <div className="w-7 h-7 rounded-lg bg-gray-100 flex items-center justify-center">
          ...
          <div className="relative" ref={accountRef}>
            ...
            {openMenu === "account" && (
              <div
                className="absolute right-0 mt-2 w-64 bg-gray-100 rounded-2xl shadow-2xl border border-gray-100 overflow-hidden z-50"
                style={{ animation: "cpSlideDown .2s ease" }}
              >
                <div className="px-4 py-4 bg-linear-to-br from-emerald-50 to-white border-b border-gray-100 flex items-center gap-3">
                  <div
