      ...
      <div className="flex flex-col gap-3 pointer-events-auto w-full max-w-md px-4">
        {alerts.map((a) => (
          <div key={a.id}
               ...
               style={{ background:"linear-gradient(135deg,#fff7ed 0%,#fff 100%)", borderColor:"#fb923c", animation:"slideDown .35s cubic-bezier(.22,1,.36,1)" }}>
            ...
            <button onClick={() => onDismiss(a.id)}
                    className="text-orange-400 hover:text-orange-700 transition-colors text-lg leading-none mt-0.5">✕</button>
          </div>
        ))}
      </div>
    </div>
...
function LogRow({ entry }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-4 rounded-xl"
      ...
      <span className="w-16 text-center text-xs font-bold uppercase tracking-widest rounded-full py-1"
...
function StatCard({ label, value, accent }) {
  return (
    <div className="rounded-xl px-5 py-6 flex flex-col justify-between shadow-sm border border-gray-100 bg-white" style={{height: "110px"}}>
...
export default function InventoryManagement() {
  ...
  const handleTransaction = () => {
    ...
    setItems((p) => p.map((i) => i.id === item.id
      ? { ...i, stock: modal.type === "IN" ? i.stock + qty : i.stock - qty } : i));
    setLog((p) => [{ id: Date.now(), type: modal.type, name: item.name, qty, unit: item.unit, time: now() }, ...p]);
    if (modal.type === "IN") setDismissed((p) => { const n = new Set(p); n.delete(item.id); return n; });
    setModal(null);
    setForm({ itemId: "", qty: "" });
  };
  ...
  const handleAddItem = () => {
    ...
    if (!newItem.minQty || parseFloat(newItem.minQty) < 0) return setFormErr("Enter a valid minimum quantity.");
    setItems((p) => [...p, { id: Date.now(), name: newItem.name.trim(), unit: newItem.unit, stock: parseFloat(newItem.stock)||0, minQty: parseFloat(newItem.minQty) }]);
        ...
        <div className="w-full bg-emerald-700 px-4 sm:px-8 py-4 flex items-center justify-between gap-3 ">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="leading-tight">
              <p className="text-2xl uppercase tracking-widest font-serif text-white" >
                Hotel Management
              </p>
              <p className=" mt-1 font-black leading-none" style={{ fontFamily:"serif", fontSize:"1.1rem", color:"#d8d8d7" }}>
                Inventory Control
              </p>
            </div>
          </div>
        ...
        <div className="w-full px-6 flex">
          {[
            ...
            <button key={key} onClick={() => setActiveTab(key)}
              ...
              {label}
              {activeTab === key && (
                <span className="absolute bottom-0 left-5 right-5 h-0.5 rounded-full"
                ...
                <div className="relative flex-1 min-w-50 max-w-sm">
                  ...
                  {search && (
                    <button onClick={() => setSearch("")}
                            className="absolute right-2.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors text-xs">✕</button>
                  )}
                </div>
                <div className="flex items-center gap-2 flex-wrap">
                  <button onClick={() => { setModal({ type:"IN" }); setFormErr(""); }}
                  ...
                  <button onClick={() => { setModal({ type:"OUT" }); setFormErr(""); }}
                          className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-xs font-bold text-white shadow-sm transition-all active:scale-95"
                  ...
                  <button onClick={() => { setAddItemModal(true); setFormErr(""); }}
                    ...
                    <div key={item.id} className="px-5 py-4 flex items-center gap-4 card-hover">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-gray-800 text-base"
                                style={{ fontFamily:"'Playfair Display',serif" }}>{item.name}</span>
                          {isLow && (
                      ...
                      <div className="text-right shrink-0">
                        <p className={`text-2xl font-black ${isLow ? "text-orange-600" : "text-gray-800"}`}
                           style={{ fontFamily:"'Playfair Display',serif" }}>{item.stock}</p>
          ...
          <div style={{ animation:"fadeIn .4s ease" }}>
            <div className="rounded-2xl overflow-hidden shadow-sm border border-gray-100 bg-white">
              ...
              {log.length === 0 ? (
                ...
                <div className="stock-scroll overflow-y-auto p-3 flex flex-col gap-2"
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                          <td className="px-4 py-3 font-semibold text-gray-700"
                              style={{ fontFamily:"'Playfair Display',serif" }}>{item.name}</td>
                          <td className="px-4 py-3 text-gray-500">{item.unit}</td>
                          <td className="px-4 py-3 font-bold"
                              style={{ color: isLow ? "#ea580c" : "#16a34a" }}>{item.stock} {item.unit}</td>
          ...
          <div className="w-full max-w-sm rounded-2xl bg-white shadow-2xl p-6"
            ...
            <div className="flex flex-col gap-3">
              <div>
                ...
                <input type="text" placeholder="e.g. Chicken" className={inp} value={newItem.name}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input type="number" min="0" step="0.1" placeholder="0" className={inp} value={newItem.stock}
                         onChange={(e) => setNewItem({ ...newItem, stock: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">Min Qty Trigger</label>
                <input type="number" min="0" step="0.1" placeholder="e.g. 5" className={inp} value={newItem.minQty}
