...
export default function InventoryManagement() {
  ...
  const handleTransaction = async () => {
    ...
    try {
      const res = await axiosInstance.patch<Item>(endpoint, { qty });
              ...
              <div style={{ animation: "fadeIn .4s ease" }}>
                ...
                <div className="max-w-6xl mx-auto">
                  <div className="rounded-2xl overflow-hidden shadow-sm border border-gray-100 bg-white">
                    ...
                    <div className="stock-scroll overflow-y-auto divide-y divide-gray-50" style={{ maxHeight: "calc(100vh - 380px)" }}>
                        ...
                        filteredItems.map((item) => {
                          ...
                          return (
                            <div key={item.id} className="px-5 py-4 flex items-center gap-4 card-hover">
                              <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl shrink-0" style={{ background: isLow ? "#fff7ed" : "#f0fdf4" }}>
                                {isLow ? "⚠️" : "✅"}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2">
                                  <span className="font-semibold text-gray-800 text-base" style={{ fontFamily: "'Playfair Display',serif" }}>{item.name}</span>
                                  {isLow && <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#fee2e2", color: "#b91c1c" }}>LOW</span>}
                                </div>
                              ...
                              <div className="text-right shrink-0">
                                <p className={`text-2xl font-black ${isLow ? "text-orange-600" : "text-gray-800"}`} style={{ fontFamily: "'Playfair Display',serif" }}>{item.stock}</p>
                        ...
                        {items.map((item) => {
                          ...
                          return (
                            <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                              <td className="px-4 py-3 font-semibold text-gray-700" style={{ fontFamily: "'Playfair Display',serif" }}>{item.name}</td>
                              <td className="px-4 py-3 text-gray-500">{item.unit}</td>
                              <td className="px-4 py-3 font-bold" style={{ color: isLow ? "#ea580c" : "#16a34a" }}>{item.stock} {item.unit}</td>
                              <td className="px-4 py-3 text-gray-500">{item.minQty} {item.unit}</td>
                              <td className="px-4 py-3">
                                <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ background: isLow ? "#fee2e2" : "#dcfce7", color: isLow ? "#b91c1c" : "#15803d" }}>
                                  {isLow ? "⚠ Low" : "✓ OK"}
                                </span>
                              </td>
                            </tr>
