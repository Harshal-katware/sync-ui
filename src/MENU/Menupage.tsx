    ...
    <div className="min-h-screen bg-gray-100 font-sans pb-20">
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-3 mt-6 mb-5">
          {([
            { label: t("menu.totalItems"), value: items.length,  gold: true  },
            { label: t("menu.veg"),        value: vegCount,      gold: false },
            { label: t("menu.nonVeg"),     value: nonVegCount,   gold: false },
          ] as { label: string; value: number; gold: boolean }[]).map((s) => (
            <div key={s.label} className="bg-white border border-[#e2d9c9] rounded-xl px-3 sm:px-5 py-3 sm:py-4">
              <p className="text-[9px] sm:text-[11px] text-[#9b8e75] uppercase tracking-[1.5px] mb-1 truncate">{s.label}</p>
              <p className={`text-xl sm:text-2xl font-medium ${s.gold ? "text-[#c9a84c]" : "text-[#1a1200]"}`}>{s.value}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 mb-3">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={openAdd} className="flex items-center gap-2 bg-[#c9a84c] hover:bg-[#b8943e] text-[#1a1200] text-[12px] sm:text-[13px] font-medium px-3 sm:px-5 py-2 sm:py-2.5 rounded-lg whitespace-nowrap">
              {t("menu.addNew")}
            </button>
          </div>
        </div>
          ...
          <div className="max-h-105 overflow-y-auto">
            <table className="w-full text-[13.5px]">
              <thead className="bg-[#faf7f0] sticky top-0 z-10">
                <tr>
                  {tableHeaders.map((h) => (
                    <th key={h} className="px-5 py-3.5 text-left text-[10.5px] font-medium text-[#b8ac9a] uppercase tracking-[1.8px] whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-12 text-[#b8ac9a] text-[14px]">{t("menu.noItems")}</td></tr>
                ) : (
                  filtered.map((item, idx) => (
                    <tr key={item.id} className="border-t border-[#f0ebe0] hover:bg-[#faf7f0] transition-colors">
                      ...
                      <td className="px-5 py-3.5 text-[#c9a84c] text-[15px] font-medium" style={{ fontFamily: "'Playfair Display', serif" }}>₹{item.price}</td>
                      ...
                      <td className="px-5 py-3.5">
                        <div className="flex gap-2">
                          <button onClick={() => openEdit(item)} className="px-3 py-1 text-[11.5px] border border-[#e2d9c9] rounded-lg text-[#6b5f50] hover:border-[#c9a84c] hover:text-[#9a7a20] hover:bg-[#c9a84c]/10 transition-colors">{t("menu.edit")}</button>
                          <button onClick={() => handleDelete(item.id)} className="px-3 py-1 text-[11.5px] border border-[#e2d9c9] rounded-lg text-[#6b5f50] hover:border-[#e74c3c] hover:text-[#c0392b] hover:bg-[#e74c3c]/10 transition-colors">{t("menu.delete")}</button>
                        </div>
                      </td>
                    </tr>
                  ))
        ...
        <div className="md:hidden space-y-2">
          {filtered.length === 0 ? (
            <div className="bg-white border border-[#e2d9c9] rounded-2xl py-12 text-center text-[#b8ac9a] text-[14px]">{t("menu.noItems")}</div>
          ) : (
            filtered.map((item, idx) => (
              <div key={item.id} className="bg-white border border-[#e2d9c9] rounded-xl px-4 py-3.5">
                ...
                <div className="flex items-center justify-between gap-2">
                  <span className="text-[#c9a84c] text-[15px] font-medium" style={{ fontFamily: "'Playfair Display', serif" }}>₹{item.price}</span>
                  <div className="flex gap-2">
                    <button onClick={() => openEdit(item)} className="px-3 py-1 text-[11.5px] border border-[#e2d9c9] rounded-lg text-[#6b5f50] hover:border-[#c9a84c] hover:text-[#9a7a20] hover:bg-[#c9a84c]/10 transition-colors">{t("menu.edit")}</button>
                    <button onClick={() => handleDelete(item.id)} className="px-3 py-1 text-[11.5px] border border-[#e2d9c9] rounded-lg text-[#6b5f50] hover:border-[#e74c3c] hover:text-[#c0392b] hover:bg-[#e74c3c]/10 transition-colors">{t("menu.delete")}</button>
                  </div>
